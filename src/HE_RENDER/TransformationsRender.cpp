#include "pch.h"
#include "TransformationsRender.h"
...
IMPLEMENT_DYNAMIC_RENDER_GROUP(CTransformationsRender, RenderType::tutorial_Transformations, RenderGroup::learn_opengl)

CTransformationsRender::CTransformationsRender()
{
}
...
void CTransformationsRender::wglInitialUpdate(IRenderEngine* pRenderEngine)
{
	auto pRenderEngineImpl = static_cast<CRenderEngine*>(pRenderEngine);
	auto pShaderManager = pRenderEngineImpl->GetShaderManager();

	m_pProgram = pShaderManager->wglGetShaderProgram(ProgramType::tutorial_5_1_transformations);
...
void CTransformationsRender::wglRelease()
{
	if (m_VAO)
...
void CTransformationsRender::wglBuild()
{
	float vertices[] = {
...
#include <chrono>
void CTransformationsRender::wglDraw()
{
